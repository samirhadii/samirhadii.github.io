...
function updateMenu() {
    ...
    // Update indicators (desktop only)
    if (!isMobile) {
...
// Click on side indicators (desktop only)
document.getElementById('leftIndicator').addEventListener('click', () => {
    if (!isMobile) navigate('prev');
});

document.getElementById('rightIndicator').addEventListener('click', () => {
    if (!isMobile) navigate('next');
});
